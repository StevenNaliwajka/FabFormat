...
class RectangleAperture(ApertureParent):
    ...
    def to_common_form(self, center_x, center_y, x_size, y_size, inside_hole_diam):
        if inside_hole_diam:
            # Starts Creation of Complex Form
            self.complex_rectangle_to_cf(center_x, center_y, x_size, y_size, inside_hole_diam)
        else:
            # Creats Simple form Polygon #1
            self.rectangle_to_cf(center_x, center_y, x_size, y_size)

    def complex_rectangle_to_cf(self, center_x, center_y, x_size, y_size, inside_hole_diam):
        # Complex Rectangle: #1,2,3,4,5
        self.create_corner_arcs_and_inside_circle(center_x, center_y, x_size, y_size, inside_hole_diam)
        ...
        if smallest == x_size:
            ...
            rect_center_y = center_y + (y_size+x_size)/4
            rect_len_x = x_size
            rect_len_y = (y_size / 2) - (x_size/2)
            self.rectangle_to_cf(rect_center_x, rect_center_y, rect_len_x, rect_len_y)
            ...
            # Y axis is smallest
            # Create RECT 6
            rect_center_x = center_x + (y_size + x_size) / 4
